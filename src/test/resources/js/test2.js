...
$(document).ready(function () {
    $.xes.require(["dropdown", "top", "label", "pop", "dom", "tip"], function () {
        if ($("#dropdownAddress").length > 0) {
            $("#dropdownAddress").dropdown({
                menu: "#address-pulldown",
                trigger: "click",
                hideTrigger: "mouseleave",
                hideHandler: "#address-pulldown"
            });
            $("#dropdownAddress").bind("click", function () {
                $(this).addClass("hover");
            });
            $("#address-pulldown").bind("mouseleave", function () {
                $("#dropdownAddress").removeClass("hover");
            });
            $("#address-pulldown li").each(function () {
                var $this = $(this);
                var $menu = $this.find(".sub-address");
                $(this).bind("mouseover", function () {
                    $("#address-pulldown .sub-address").hide();
                });
                $(this).dropdown({menu: $menu, trigger: "mouseover", hideTrigger: "mouseleave", hideHandler: $menu});
            });
        }
        $("#dropdownTime").dropdown({
            ...
            trigger: "click",
            hideTrigger: "mouseleave",
            hideHandler: "#time-pulldown"
        });
        $("#dropdownTime").bind("click mouseover", function () {
            $(this).addClass("hover");
        });
        $("#time-pulldown").bind("mouseleave", function () {
            $("#dropdownTime").removeClass("hover");
        });
        $("#time-pulldown li").each(function () {
            var $this = $(this);
            var $menu = $this.find(".sub-address");
            $(this).bind("mouseover", function () {
                $("#time-pulldown .sub-address").hide();
            });
            $(this).dropdown({menu: $menu, trigger: "mouseover", hideTrigger: "mouseleave", hideHandler: $menu});
        });
        $("#tb-site").bind("mouseover mouseleave", function (e) {
            if (e.type === "mouseover") {
        ...
        $("#tb-site").dropdown({menu: $("#tb-site-list"), trigger: "mouseover", hideTrigger: "mouseleave"});
        $("#search-expand-btn").bind("click", function (e) {
            if ($(this).find("span").is(".hover")) {
                $(this).find("span").removeClass("hover");
            } else {
                $(this).find("span").addClass("hover");
            }
        });
        $("#search-expand-btn").dropdown({menu: "#search-term", trigger: "click"});
        $("#footExtend").bind("click", function (e) {
            if ($(this).is(".hover")) {
        ...
        $("#footExtend").dropdown({menu: "#setClass-box", trigger: "click"});
        $("#setClassBar").bind("click", function (e) {
            var arrow = $("#footExtend");
            if (arrow.is(".hover")) {
        ...
        $("#setClassBar").dropdown({menu: "#setClass-box", trigger: "click"});
        $(".ico-list").each(function () {
            var $this = $(this);
            $this.find("span.last").dropdown({menu: $this.parents(".s-r-list").find(".comment-box"), trigger: "click"});
            $this.find("span").eq(2).click(function () {
                if ($(this).find("a").length > 0) {
                    $this.parents(".s-r-list").find(".tip").fadeIn();
                }
            });
        });
        if ($("#go_top").length > 0) {
            $("#go_top").top({
                cssText: "display:block; bottom:55px;",
                targetObj: $("section.sk-wrapper").eq(0),
                viewPos: 100
            });
        }
        $("div.course-infor span.choose").each(function () {
            $(this).dropdown({menu: $(this).parents("div.s-r-list").find(".timetable"), trigger: "click"});
        });
        $(".schedule").each(function () {
            var $this = $(this);
            var $container = $this.parents(".s-r-list");
            var $timetable = $container.find(".timetable");
        ...
        if ($("#loginALink").length > 0) {
            $("#loginALink").pop({dialog: ".login-box", closeBtn: "#closeBtn"});
        }
        if ($('#CasUsername').length > 0) {
            $("#CasUsername").label({label: "#labelForUserName"});
        }
    });
    $(".mtop10 span.comment-expand").click(function () {
        $(this).parent().parent().hide();
    });
    $(".sk-bm-btn").each(function () {
        $(this).attr('btnEnabled', 'true');
    });
    $(".sk-bm-btn").bind("click", function (e) {
        var $this = $(this);
    ...
    $(".menu-level1>a").click(function () {
        ...
        addMarkerArea($(this).attr('id'));
        return true;
    });
    $("#menu-all>a").click(function () {
        $(this).addClass("current").parent().siblings().children("a").removeClass("current").next().hide();
        return false;
    });
    $(".table-setClass>tbody>tr").hover(function () {
        $(this).addClass("hover");
    }, function () {
        $(this).removeClass("hover");
    });
    $(".g-rule").hover(function () {
        $(this).find("div").show();
    }, function () {
        $(this).find("div").hide();
    });
    $("#pay-tip01 b a").click(function () {
        $("#pay-tip01").hide();
    });
    var B_cookie_name = 'speiyou_login_name';
    var user_name = $.xes.cookie.get(B_cookie_name);
    if (user_name != null) {
        if ($('#labelForUserName').length > 0) {
            $('#labelForUserName').html('');
        }
        $("#CasUsername").val(user_name);
        if (document.getElementById("remaindbox") != null) {
            document.getElementById("remaindbox").checked = true;
        }
    } else {
        if ($('#labelForUserName').length > 0) {
            $('#labelForUserName').html('使用手机/邮箱/学员编号进行登录');
        }
    }
    $('#CasUsername').click(function () {
        $('#labelForUserName').hide();
    });
    $("#loginBtn").bind("click", function () {
        var userName = $.trim($("#CasUsername").val());
        var userPwd = $.trim($("#CasPassword").val());
        var yzm = $.trim($("#CasValidcode").val());
        var token = $('#CasToken').val();
        var $errInfo = $("#errInfo");
        var errInfoStr = "";
        if ($("#yzmTr").is(":visible")) {
            if (userName && userName != 'undefined' && userPwd && userPwd != 'undefined' && yzm && yzm != 'undefined') {
                var agree = $('#remaindbox')[0].checked;
                var B_cookie_name = 'speiyou_login_name';
                if (agree !== false) {
                    $.xes.cookie.set(B_cookie_name, userName, '/', 720);
                } else {
                    $.xes.cookie.del(B_cookie_name);
                }
                $.xes.cookie.set('B_cookie_login_status', 'begin', '/', 0.1);
                $.ajax({
                    type: 'post',
                    url: '/Users/validLoginUser',
                    data: 'username=' + userName + '&password=' + userPwd + "&token=" + token + "&validcode=" + yzm + "&timer=" + new Date().getTime(),
                    dataType: 'json',
                    success: function (json) {
                        if (typeof(json.errorNum) != 'undefined' && parseInt(json.errorNum) >= 3) {
                            $('#verifyimg').attr('src', '/Users/securImage/0?' + Math.random());
                            $('#yzmTr').removeClass('hide');
                        }
                        switch (json.tip) {
                            case'ok':
                                var randomNum = Math.random();
                                var scriptUrl = casActUrl + "/?fs=" + pyUrl + "&t=" + randomNum;
                                ...
                                break;
                            case'codeError':
                                $("#errInfo").text("验证码错误,请重新输入").show();
                                break;
                            case'numError':
                                $("#errInfo").text("错误次数过多,请" + json.time + "分钟后重新登录").show();
                                break;
                            case'tokenError':
                                $("#errInfo").text("数据不合法，请刷新页面重新登录").show();
                                break;
                            case'subError':
                                $("#errInfo").text("非法提交").show();
                                break;
                            case'loginError':
                                $("#errInfo").text("用户名和密码不匹配").show();
                                break;
                        }
                    }
                });
                ...
                if (!userName && !userPwd && !yzm) {
                    errInfoStr = "请输入用户名,密码和验证码";
                } else {
                    if (userName) {
                        if (!userPwd && !yzm) {
                            errInfoStr = "请输入密码和验证码";
                        } else if (userPwd) {
                            errInfoStr = "请输入正确的验证码";
                        } else {
                            errInfoStr = "请输入密码";
                        }
                    } else if (userPwd) {
                        if (!userName && !yzm) {
                            errInfoStr = "请输入用户名和验证码";
                        } else if (userName) {
                            errInfoStr = "请输入正确的验证码";
                        } else {
                            errInfoStr = "请输入用户名";
            ...
            if (userName && userName != 'undefined' && userPwd && userPwd != 'undefined') {
                ...
                $.ajax({
                    type: 'post',
                    url: '/Users/validLoginUser',
                    data: 'username=' + userName + '&password=' + userPwd + "&token=" + token + "&timer=" + new Date().getTime(),
                    dataType: 'json',
                    success: function (json) {
                        if (typeof(json.errorNum) != 'undefined' && parseInt(json.errorNum) >= 3) {
                            $('#verifyimg').attr('src', '/Users/securImage/0?' + Math.random());
    ...
    $("#copyright_year").html(vYear);
    $(document).bind("click", function (e) {
        if (!($(e.target).is("#dropdownAddress") || $(e.target).is("#address-pulldown"))) {
            $("#address-pulldown").hide();
        }
    });
});
...
;(function ($) {
    ...
    $.xes.template = function () {
        ...
        for (var i = 0, l = templateScript.length; i < l; i += 1) {
            templates[$(templateScript[i]).attr("forObj")] = $.trim($(templateScript[i]).html());
        }
        return templates;
    };
    $.xes.require = function (names, fun) {
        ...
        if (typeof names === "string") {
            ...
            script.src = CDNURL + "/js/xes." + name + ".js";
            ...
            for (var i = 0; i < total; i += 1) {
                ...
                scripts[i].src = CDNURL + "/js/xes." + names[i] + ".js";
    ...
    $.xes.getDialogPosition = function (pop) {
        ...
        return {left: (width - popWidth) / 2 + scrollLeft, top: (height - popHeight) / 2 + scrollTop};
    };
})(jQuery);
;$.xes.cookie = function () {
    return {
        getAll: function () {
            ...
            var B = $.xes.cookie.get(key);
            document.cookie = key + "=" + B + "; expires=" + C.toGMTString() + "; path=/";
        }
    };
}();
;(function ($) {
    $.fn.dialog = function (config) {
        var $this = this, win, title, href = (typeof(this[0]) != 'undefined') ? $this[0].href : '', width, height, op, id, bg, $winDom, $winContent, $closeBtn, iframe, option = {
            winDom: "",
            winContent: "",
            closeBtn: "",
            height: 500,
        ...
        $.xes.returnMap = {};
        $.xes.dialogMap = {};
        op = $.extend({}, option, config);
        $winDom = $("<div></div>");
        $this.click(function (e) {
            $winDom.html("");
            iframe = $("<iframe />");
            iframe[0].src = href;
            iframe.css({"width": op.width + "px", "border": "none", "height": +op.height + "px"});
            $winDom.append(iframe);
            bg = new $.xes.createBackGround();
            id = new Date().getTime();
            e.preventDefault();
            $(document.body).append(bg.bgDom);
            $(document.body).append($winDom);
            bg.bgDom.show();
            $winDom.show();
            $winDom.css({"position": "absolute", "z-index": "1000"});
            $.xes.returnMap[id] = op.callback;
            $.xes.dialogMap[id] = {winDom: $winDom, destroy: destroy, rePosDialog: rePosDialog};
        ...
        function rePosDialog() {
            var pos = getDialogPosition($winDom);
            $winDom.css("top", pos.top + "px");
            $winDom.css("left", pos.left + "px");
        }
        ...
        function getDialogPosition(pop) {
            ...
            var scrollTop = document.documentElement.scrollTop || window.pageYOffset || document.body.scrollTop;
            var scrollLeft = document.documentElement.scrollLeft || window.pageXOffset || document.body.scrollLeft;
            var popWidth = pop.outerWidth() + parseInt(pop.css("padding-left"), 10) + parseInt(pop.css("padding-right"), 10);
            var popHeight = pop.outerHeight() + parseInt(pop.css("padding-top"), 10) + parseInt(pop.css("padding-bottom"), 10);
            return {left: (width - popWidth) / 2 + scrollLeft, top: (height - popHeight) / 2 + scrollTop};
        }
        ...
        function destroy() {
            bg.bgDom.remove();
            $winDom.hide();
            delete $.xes.returnMap[id];
            delete $.xes.dialogMap[id];
        }
    };
})(jQuery);
;function cancelRegist(claId) {
    if (confirm("您确定要删除此班级吗？")) {
        $.ajax({
            type: 'post',
            url: '/registers/cancelRegist',
            data: 'claId=' + claId + "&time=" + new Date().getTime(),
            dataType: 'json',
            success: function (res) {
                if (res.msg == '10000') {
                    cancelHuanxunClassRelease(claId);
                    alert('恭喜您，撤单成功');
                    location.reload();
                } else {
                    alert(res.msg);
...
function cancelHuanxunClassRelease(claId) {
    $.ajax({
        type: 'post',
        url: '/registers/cancelHuanxunClassRelease',
        data: 'claId=' + claId + "&time=" + new Date().getTime(),
        dataType: 'json',
        success: function (res) {
            if (res.msg == '10000') {
...
function checkClass() {
    ...
    objId = document.getElementsByName('checkbox2');
    if (myCheckbox.checked) {
        for (var band = 0; band < objId.length; band++) {
            objId[band].checked = true;
        }
    } else {
        for (var band = 0; band < objId.length; band++) {
            objId[band].checked = false;
        }
    }
}
function goRegistConfirm() {
    var payCheckBox = document.getElementById('payCheckBox');
    if (payCheckBox.checked == false) {
        alert('请同意报名退费协议');
        return false;
    }
    var objId = document.getElementsByName('checkbox2');
    var arr = new Array();
    if (objId.length > 0) {
        for (var band = 0; band < objId.length; band++) {
            if (objId[band].checked) {
                arr.push($(objId[band]).attr('id'));
            }
        }
        if (arr.length == 0) {
            alert("请选择缴费的班级！");
        } else {
            window.location.href = '/registers/confirm/' + arr;
            return false;
        }
    } else {
        alert("请选择缴费的班级！");
...
function showYhj(obj) {
    var yhqDiv = $(obj).parent().parent().parent().parent().next();
    yhqDiv.toggle();
    if (yhqDiv.is(":hidden")) {
        $(obj).attr('class', '');
    } else {
...
function checkOffersCouponNum(claId, obj, sign1) {
    if (claId && obj) {
        var inputCard = $(obj).parent().prev().children().first();
        var card = $.trim(inputCard.val());
        var tsDiv = $(obj).parent().parent().next().next();
        if (card == '') {
            tsDiv.html('优惠券编号不能为空');
        } else {
            var sign = 0;
            $('.text02').each(function () {
                if ($(this).val() == card) {
                    ...
                    if (sign == 2) {
                        tsDiv.html('优惠券号不能重复');
                        $(obj).parent().prev().children().first().val('');
                    }
                }
            });
            if (sign > 1) {
            ...
            var sameCard
            if (sign1 == 1) {
                sameCard = $(obj).parents(".coupon-bar").find("input").eq(1).val();
            } else {
                sameCard = $(obj).parents(".coupon-bar").find("input").eq(0).val();
            }
            $.ajax({
                type: "POST",
                url: "/registers/checkCoupon",
                data: "claId=" + claId + "&card=" + card + "&sameCard=" + sameCard + "&time=" + t,
                dataType: "json",
                success: function (msg) {
                    if (msg.sta) {
                        if (msg.msg != 0) {
                            tsDiv.html('优惠券有效！优惠金额：' + msg.msg);
                        } else {
                            tsDiv.html('优惠券无法优惠报名费');
                        }
                        $("#couponPrice").html(parseInt($("#couponPrice").html()) + parseInt(msg.msg));
                        $("#payClassPrice").html(parseInt($("#payClassPrice").html()) - msg.msg);
                        $("#lastPrice").html(parseInt($("#lastPrice").html()) - msg.msg);
                    } else {
                        tsDiv.html(msg.msg);
                    }
                },
...
function getcity(p_id) {
    ...
    $.ajax({
        type: "POST",
        url: "/Students/resddress/",
        data: "type=city&p_id=" + p_id + "&time=" + t,
        dataType: "json",
        success: function (json) {
            if (json.msg === true) {
                $("#c_select").html('<option value="">选择城市</option>' + json.str);
            } else {
                alert("请求失败，请重试！");
            }
        }
...
function getdistrict(c_id) {
    if (!c_id)return false;
    var t = new Date().getTime();
    $.ajax({
        type: "POST",
        url: "/Students/resddress/",
        data: "type=district&c_id=" + c_id + "&time=" + t,
        dataType: "json",
        success: function (json) {
            if (json.msg === true) {
                $("#d_select").html('<option value="">选择县区</option>' + json.str);
            } else {
                alert("请求失败，请重试！");
            }
        }
...
function deltri(tri_id, obj) {
    ...
    if (window.confirm("确定删除吗？")) {
        ...
        $.ajax({
            type: "POST",
            url: "/Students/deltri/",
            data: "tri_id=" + tri_id + "&t=" + time,
            dataType: "json",
            success: function (json) {
                if (json.msg === true) {
                    par = $(obj).parent().parent();
                    lin = $(obj).parent().parent().next();
                    par.remove();
                    lin.remove();
                    alert("删除成功！");
                } else {
...
function addtri() {
    ...
    var p_select = $("#p_select").val();
    var c_select = $("#c_select").val();
    var d_select = $("#d_select").val();
    var address = $("#address").val();
    var zip = $("#zip").val();
    var call = $("#call").val();
    var phone = $("#phone").val();
    var t = new Date().getTime();
    var token = '4028873d27e0dbe40127e0e1136dtfrd';
    var regStr = $("#regStr").val();
    if (!repname) {
        alert("请填写收件人信息");
        return false;
    }
    if (!(p_select && c_select && d_select)) {
        alert("请填写配送地区信息");
        return false;
    }
    if (!address) {
    ...
    if (call == '' && phone == '') {
        alert("至少填写手机或固定电话中的一项");
        return false;
    }
    if (repname.length < 2 || repname.length > 10) {
        alert("收货人名称长度2～10个");
        return false;
    }
    ...
    $.ajax({
        type: "POST",
        url: "/Students/addtri/",
        data: "repname=" + repname + "&p_select=" + p_select + "&c_select=" + c_select + "&d_select=" + d_select + "&address=" + address + "&zip=" + zip + "&call=" + call + "&phone=" + phone + "&token=" + token + "&time=" + t,
        dataType: "json",
        success: function (json) {
            if (json.msg == true) {
                $("#repname").val('');
                ...
                $("#call").val('');
                $("#phone").val('');
                var html = '<div class="pdboth01"><span class="fn-right"><a onclick="deltri(\'' + json.uid + '\',this)" href="javascript:void(0)">删除</a></span>';
                html += '<span><input type="radio" value="radio" id="' + json.uid + '" onclick="javascript:s_address(\'' + regStr + '\', this.id);" name="addInfRadio"></span><span class="wid120">收件人：<font class="ftgray02 ft12">' + repname + '</font></span>';
                html += '<span>配送地址：<font class="ftgray02 ft12">' + address + '</font></span></div><div class="spaceLine02"></div>';
                $("#adsListDiv").append(html);
                alert("保存收件地址成功！");
                return;
            } else {
                alert(json.error);
...
function checkPay() {
    var centerRadio = $("#centerRadio")[0].checked;
    var devRadio = $("#devRadio")[0].checked;
    if (centerRadio || devRadio) {
        if (devRadio) {
            var radioCheck = 0;
            ...
            if (radioCheck == 0) {
                ...
                var addInfRadioError = 0;
                $('input[name="addInfRadio"]').each(function () {
                    if (this.checked) {
                        addInfRadioError++;
                    }
                });
                if (addInfRadioError == 0) {
                    alert('请选择收件人信息');
                    return;
                } else {
                    $("#payForm").submit();
...
function s_address(reg_id, tri_id) {
    if (!reg_id)return false;
    $.ajax({
        type: "POST",
        url: "/Pay/saddress/",
        data: "reg_id=" + reg_id + "&tri_id=" + tri_id,
        dataType: "json",
        success: function (json) {
        }
    });
}
function payOrderSub(arr, orderId) {
    var payCheckBox = document.getElementById('payCheckBox');
    if (payCheckBox.checked == false) {
        alert('请同意报名退费协议');
        return false;
    ...
    if (arr) {
        $.ajax({
            type: "POST",
            url: "/Registers/checkOrder",
            data: "orderId=" + orderId,
            dataType: "json",
            success: function (json) {
                if (json.msg == 'sucess') {
                    window.location.href = '/registers/confirm/' + arr;
                } else if (json.msg == 'orderError') {
                    alert('订单有误！');
                    window.location.reload();
                } else if (json.msg == 'orderDelete') {
                    alert('订单已被删除！');
                    window.location.reload();
                } else if (json.msg == 'orderExpired') {
                    alert('订单已失效！');
                    window.location.reload();
                } else if (json.msg == 'notPay') {
                    alert('订单缴费处理中，请1分钟后再试！');
                } else {
                    alert('服务器错误！');
                    window.location.reload();
                }
...
function conPayMethod() {
    var showLi = 1;
    $("ul.pay-mode-tab").children().each(function (i) {
        if ($(this).attr('class') == 'hover') {
            showLi = i + 1;
        }
    });
    if (showLi == 1) {
        var stuCardPwd = $.trim($("#cardPayInput").val());
        var orderId = $.trim($("#orderId").val());
        var isPayEn = $.trim($("#isPayEn").val());
        if (orderId == '') {
            alert('参数缺失！');
            return false;
        }
        ...
        if (stuCardPwd != '') {
            price = parseInt($("#priceSpan").html());
            $.ajax({
                type: "POST",
                url: "/Pay/checkStuCard",
                data: "stuCardPwd=" + stuCardPwd + "&price=" + price,
                dataType: "json",
                success: function (json) {
                    if (json.result) {
                        window.location.href = "/Pay/cardPay/" + orderId;
                    } else {
                        alert(json.msg);
                        $("#cardPayInput").val('');
                    }
        ...
        var fastDebitPays = $("input[name='fast_debit_pay']"), i, l;
        var bankDebitPays = $("input[name='bank_debit_pay']");
        var fastDebitPaysSta = false;
        var bankDebitPaysSta = false;
        for (i = 0, l = fastDebitPays.length; i < l; i += 1) {
            if (fastDebitPays.eq(i)[0].checked) {
                fastDebitPaysSta = true;
            }
        }
        if (!fastDebitPaysSta) {
            for (i = 0, l = bankDebitPays.length; i < l; i += 1) {
                if (bankDebitPays.eq(i)[0].checked) {
                    bankDebitPaysSta = true;
                }
            }
        }
        if (fastDebitPaysSta || bankDebitPaysSta) {
            var pyOrderId = $.trim($("#pyOrderId").val());
            var t = new Date().getTime();
            if (pyOrderId == '') {
                return false;
            } else {
                $.ajax({
                    type: "POST",
                    url: "/Pay/updatePayState/",
                    data: "pyOrderId=" + pyOrderId + "&time=" + t,
                    dataType: "json",
                    success: function (msg) {
                        if (msg.msg == true) {
                            if (fastDebitPaysSta) {
                                $("#fastPay_debit_alipaysubmit").submit();
                            } else {
                                $("#bankPay_debit_alipaysubmit").submit();
                            }
                        } else {
                            alert('修改支付时间失败，请刷新页面重试');
                            return;
                        }
                    }
                });
        ...
        var fastCreditPays = $("input[name='fast_credit_pay']"), i, l;
        var bankMixPay = $("input[name='bank_mix_pay']");
        var fastCreditPaysSta = false;
        var bankMixPaySta = false;
        for (i = 0, l = fastCreditPays.length; i < l; i += 1) {
            if (fastCreditPays.eq(i)[0].checked) {
                fastCreditPaysSta = true;
            }
        }
        if (!fastCreditPaysSta) {
            for (i = 0, l = bankMixPay.length; i < l; i += 1) {
                if (bankMixPay.eq(i)[0].checked) {
                    bankMixPaySta = true;
                }
            }
        }
        if (fastCreditPaysSta || bankMixPaySta) {
            var pyOrderId = $.trim($("#pyOrderId").val());
            var t = new Date().getTime();
            if (pyOrderId == '') {
                return false;
                ...
                $.ajax({
                    ...
                    success: function (msg) {
                        if (msg.msg == true) {
                            if (fastCreditPaysSta) {
                                $("#fastPay_credit_alipaysubmit").submit();
                            } else {
                                $("#bankPay_mix_alipaysubmit").submit();
                            }
                        } else {
                            alert('修改支付时间失败，请刷新页面重试');
                            return;
        ...
        var payPlatZfb = $("input[name='pay_plat_zfb']"), i, l;
        var payPlatZfbSta = false;
        for (i = 0, l = payPlatZfb.length; i < l; i += 1) {
            if (payPlatZfb.eq(i)[0].checked) {
                payPlatZfbSta = true;
            }
        }
        if (payPlatZfbSta) {
...
function bankPay(bank, type) {
    if (type == 1) {
        if ($("input[type='radio'][name='fast_debit_pay']:checked")[0] !== undefined) {
            var fast_debit_pay = $("input[type='radio'][name='fast_debit_pay']:checked")[0];
            fast_debit_pay.checked = false;
        }
    }
    if (type == 2) {
        if ($("input[type='radio'][name='fast_credit_pay']:checked")[0] !== undefined) {
            var fast_credit_pay = $("input[type='radio'][name='fast_credit_pay']:checked")[0];
            fast_credit_pay.checked = false;
        }
    }
    var t = new Date().getTime();
    if (bank == '') {
        ...
        $(".pay-btn-orange02").hide();
        $.ajax({
            type: "POST",
            url: "/Pay/bankPayAjax/",
            data: "bank=" + bank + "&type=" + type + "&time=" + t,
            success: function (msg) {
                if (type == 1) {
                    $("#bankPay_debit_sHtmlText").html(msg);
                } else {
                    $("#bankPay_mix_sHtmlText").html(msg);
                }
                $(".pay-btn-orange02").show();
            }
        });
    }
}
function fastPay(bank, type) {
    if (type == 1) {
        if ($("input[type='radio'][name='bank_debit_pay']:checked")[0] !== undefined) {
            var bank_debit_pay = $("input[type='radio'][name='bank_debit_pay']:checked")[0];
            bank_debit_pay.checked = false;
        }
    }
    if (type == 2) {
        if ($("input[type='radio'][name='bank_mix_pay']:checked")[0] !== undefined) {
            var bank_mix_pay = $("input[type='radio'][name='bank_mix_pay']:checked")[0];
            bank_mix_pay.checked = false;
        }
    }
    var t = new Date().getTime();
    if (bank == '') {
        ...
        $.ajax({
            ...
            success: function (msg) {
                if (type == 1) {
                    $("#fastPay_debit_sHtmlText").html(msg);
                } else {
                    $("#fastPay_credit_sHtmlText").html(msg);
                }
                $(".pay-btn-orange02").show();
            }
        });
...
function showMobbyDiv() {
    var checkButton = document.getElementById('isMobbyRegfee');
...
function checkMobbyCard() {
    var mobbyCard = $.trim($("#mobbyCard").val());
    var mobbyPassword = $.trim($("#mobbyPassword").val());
    var t = new Date().getTime();
    if (mobbyCard == '' || mobbyPassword == '') {
        alert('学员卡号与密码都为必填项！');
        return false;
    }
    $.ajax({
        type: 'post',
        url: '/registers/ifMobbyRegFee/',
        data: 'mobbyCardNo=' + mobbyCard + '&mobbyCardPassword=' + mobbyPassword + '&time=' + t,
        success: function (res) {
            if (res == 'oldStudent') {
                window.alert('由于您已在摩比缴过报名费，您可以享受报名费优惠！');
                window.location.reload();
                return false;
            }
            var str = "";
...
;(function ($) {
    $.fn.slider = function (options) {
        ...
        this.opts = opts;
        var $this = $(this), that = this, container = $this.find(opts.containerCls), items = container.find(opts.itemCls), pages = $this.find(opts.pageCls), btnPrev = $this.find(opts.btnPrevCls), btnNext = $this.find(opts.btnNextCls);
        this.container = container;
        this.items = items;
        if (!container || !items)return false;
        var itemSize = (opts.direction != 'vertical') ? items.outerWidth() : items.outerHeight(), total = items.length, perItem = opts.perItem, current = -1, playTimer;
        this.busy = false;
        this.total = total;
        if (total <= perItem) {
        ...
        this.show = function (index) {
            ...
            index = Math.max(0, Math.min(index, parseInt((this.items.length - 1) / opts.moveStep)));
            if (pages.length)pages.eq(current).removeClass(opts.activeCls);
            var fxOpts = {};
            fxOpts[(opts.direction != 'vertical') ? 'left' : 'top'] = -index * itemSize * opts.moveStep;
            if (current !== -1) {
                this.busy = true;
        ...
        function showNext() {
            if (current * opts.moveStep + perItem < that.total)that.show(current + 1); else if (opts.circle)that.show(0); else return;
        }

        function showPrev() {
            if (current > 0)that.show(current - 1); else if (opts.circle && that.total >= perItem)that.show(that.total - perItem); else return;
        }
        ...
        this.resetItems = opts.resetItems;
        return this;
    };
})(jQuery);
;$(document).ready(function () {
    $.xes.require(["slider"], function () {
        var sliderTeacher = $("#v_content").slider({
            btnPrevCls: 'a.aleft',
            btnNextCls: 'a.aright',
            containerCls: '.v_content_list ul',
            itemCls: 'li',
            perItem: 8,
            startIndex: 0,
            autoPlay: false,
            ...
            resetItems: function () {
                ...
                var opts = this.opts;
                var items = container.find(opts.itemCls + ":visible");
                var itemSize = (opts.direction != 'vertical') ? items.outerWidth() : items.outerHeight();
                this.total = items.length;
                this.items = items;
                container.css('left', 0);
                container.css((opts.direction != 'vertical') ? 'width' : 'height', itemSize * this.total);
            }
        });
        $(".s-t-header ul li").bind("click", function (e) {
            var clickClass = $(this).attr('class');
            var displayClass = "list_" + clickClass;
            $("." + displayClass).siblings().hide();
            $("." + displayClass).show();
            $("." + clickClass).siblings().removeClass('hover');
            $("." + clickClass).addClass('hover');
        });
    });
    $("div.tn-ico-list a.attention").click(function () {
        var thisElement = $(this);
        var claId = thisElement.attr('id');
        $.ajax({
            type: "POST",
            url: "/classes/addAttention",
            data: "claId=" + claId,
            dataType: "json",
            success: function (msg) {
                if (msg.tip == 'n') {
                    thisElement.tip({tipClass: "bm-tip01 fixpng", text: msg.msg, delay: 500});
                } else if (msg.tip == 'nologin') {
                    showLoginDiv();
                } else if (msg.tip == 'y') {
                    thisElement.tip({tipClass: "bm-tip01 fixpng", text: msg.msg, delay: 500});
                    var num = thisElement.find('i.attention_num').html();
                    var newNum = parseInt(num) + 1;
                    thisElement.find('.attention_num').html(newNum);
                }
            }
        });
    });
    $("#con_one_1 span.t_but_gz").click(function () {
        var thisElement = $(this);
        var claId = thisElement.attr('id');
        $.ajax({
            type: "POST",
            ...
            dataType: "json",
            success: function (msg) {
                thisElement.tip({tipClass: "bm-tip03 fixpng", text: msg.msg, delay: 500});
            }
        });
    });
});
function oncheckTime(grade, subject, level, service, gtype, term, period, o, teaid, bg, ctype, nu, e) {
    var strTime = '/time:';
    var nowValue = $(e).attr('value');
    var nowClass = $(e).attr('class');
    var findValue = new Array();
    $(e).parent().find(".selected").each(function (i) {
        if ($(this).attr('value') != undefined) {
            findValue[$(this).attr('value')] = $(this).attr('value');
        }
    });
    if (nowClass == 'selected') {
        var beginNum = nowValue;
        findValue.splice(beginNum, 1);
    } else if (nowClass == 'noselected') {
        findValue[nowValue] = nowValue;
    }
    var findValueStr = '';
    for (var i = 0; i < findValue.length; i++) {
        if (findValue[i] != undefined) {
            findValueStr += ',' + findValue[i];
        }
    }
    findValueStr = findValueStr.substring(1, findValueStr.length);
    strTime += findValueStr;
    location.href = "/search/index" + strTime + "/grade:" + grade + "/subject:" + subject + "/level:" + level + "/teaid:" + teaid + "/service:" + service + "/bg:" + bg + "/gtype:" + gtype + "/term:" + term + "/period:" + period + "/o:" + o + "/ctype:" + ctype + "/nu:" + nu;
}
function oncheckBegin(grade, subject, level, service, gtype, term, period, bg, teaid, time, ctype, nu, e) {
    var nowValue = $(e).attr('value');
    var nowClass = $(e).attr('class');
    var strBegin = '/bg:';
    if (nowClass == 'no') {
    ...
    location.href = "/search/index" + strBegin + "/grade:" + grade + "/subject:" + subject + "/level:" + level + "/teaid:" + teaid + "/service:" + service + "/gtype:" + gtype + "/time:" + time + "/term:" + term + "/period:" + period + "/ctype:" + ctype + "/nu:" + nu;
}
function oncheckNumber(grade, subject, level, service, gtype, term, period, bg, teaid, time, ctype, nu, e) {
    var nowValue = $(e).attr('value');
    var nowClass = $(e).attr('class');
    var strBegin = '/nu:';
    if (nowClass == 'no') {
    ...
    location.href = "/search/index" + strBegin + "/grade:" + grade + "/subject:" + subject + "/level:" + level + "/teaid:" + teaid + "/service:" + service + "/gtype:" + gtype + "/time:" + time + "/term:" + term + "/period:" + period + "/ctype:" + ctype + "/bg:" + bg;
}
function checkSearch(e) {
    var searchVal = $("#search_kw").val();
    searchVal = $.trim(searchVal);
    if ((searchVal.length < 1) || (searchVal == '输入关键字查找班级')) {
        $(e).tip({tipClass: "bm-tip03 fixpng", text: '请输入搜索的课程信息', delay: 500});
        return false;
    }
    return true;
}
;(function ($) {
    $.fn.tip = function (cfg) {
        var option = {tipClass: "", text: "", delay: 1000};
        var op = $.extend({}, option, cfg);
        var $this = $(this);
        var top, left, thisTop, thisLeft, tipWidth, tipHeight, $tip;
        thisTop = $this.offset().top;
        thisLeft = $this.offset().left;
        $tip = $("<div class='" + op.tipClass + "' style='visibility:hidden'>" + op.text + "</div>");
        $(document.body).append($tip);
        tipWidth = $tip.width() + parseInt($tip.css("padding-left"), 10) + parseInt($tip.css("padding-right"), 10);
        tipHeight = $tip.height() + parseInt($tip.css("padding-top"), 10) + parseInt($tip.css("padding-bottom"), 10);
        $tip.css({
            "top": thisTop - tipHeight + "px",
            "left": (thisLeft + $this.width() / 2 - tipWidth / 2) + "px",
            "visibility": "visible",
            "display": "none",
            "z-index": 999
        });
        $tip.fadeIn(op.delay, function () {
            setTimeout(function () {
                $tip.fadeOut(op.delay, function () {
                    $tip.remove();
                });
            }, op.delay);
        });
